...
export default function EducationCMS() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["edu-list"],
    queryFn: async () => {
      const { data, error } = await supabase.from("educational_posts").select("*").order("updated_at", { ascending: false });
  ...
  const setStatusOf = async (id: string, next: string) => {
    ...
    const { error } = await supabase.from("educational_posts").update(patch).eq("id", id);
  ...
  const remove = async (id: string) => {
    ...
    const { error } = await supabase.from("educational_posts").delete().eq("id", id);
    if (error) return toast.error(error.message);
  ...
  const save = async () => {
    ...
    if (editing.id) {
      ...
      const { error } = await supabase.from("educational_posts").update(payload).eq("id", editing.id);
      if (error) return toast.error(error.message);
    } else {
      payload.author_id = user?.id;
      if (payload.status === "published") payload.published_at = new Date().toISOString();
      const { error } = await supabase.from("educational_posts").insert(payload);
      if (error) return toast.error(error.message);
    }
    toast.success("Saved");
  ...
  return (
    <div className="p-8">
      <header className="mb-6">
        ...
        <h1 className="font-display text-2xl font-semibold">Educational Content</h1>
      </header>
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead className="w-24 text-right">Approve</TableHead>
              <TableHead className="w-24 text-right">Decline</TableHead>
              <TableHead className="w-12"></TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {isLoading && <TableRow><TableCell colSpan={7} className="text-center text-muted-foreground py-8">Loading…</TableCell></TableRow>}
            {!isLoading && paged.length === 0 && <TableRow><TableCell colSpan={7} className="text-center text-muted-foreground py-8">No posts yet. Click "New" to create one.</TableCell></TableRow>}
            {paged.map((r) => (
              <TableRow key={r.id} className="cursor-pointer" onClick={() => openEdit(r)}>
                ...
                <TableCell className="text-right" onClick={(e) => e.stopPropagation()}>
                  <Button size="sm" variant="ghost" disabled={!canManage || r.status === "published"} onClick={() => setStatusOf(r.id, "published")}>
                    <Check className="h-4 w-4 text-emerald-600" />
                  </Button>
                </TableCell>
                <TableCell className="text-right" onClick={(e) => e.stopPropagation()}>
                  <Button size="sm" variant="ghost" disabled={!canManage || r.status === "declined"} onClick={() => setStatusOf(r.id, "declined")}>
      ...
      <Sheet open={open} onOpenChange={setOpen}>
        <SheetContent side="right" className="w-full sm:max-w-2xl overflow-y-auto">
          <SheetHeader><SheetTitle>{editing?.id ? "Edit post" : "New post"}</SheetTitle></SheetHeader>
          {editing && (
