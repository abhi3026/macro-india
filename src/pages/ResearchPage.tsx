    ...
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <PageHero
        ...
        aside={
          <div className="rounded-xl border border-white/15 bg-white/5 backdrop-blur p-6">
            ...
            <h2 className="font-display text-lg font-semibold mt-1 text-white">What we research</h2>
      ...
      <main className="flex-grow">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 lg:py-14 space-y-10">
          ...
          {/* Featured + sidebar, horizontally aligned */}
          <div className="grid lg:grid-cols-12 gap-6 lg:gap-8 items-stretch">
            {featured && (
              <article className="surface overflow-hidden lg:col-span-8 flex flex-col">
                <div className="grid md:grid-cols-2 flex-1">
                  {featured.image && (
                    <div className="aspect-[16/10] md:aspect-auto md:h-full overflow-hidden">
                      <img src={featured.image} alt={`${featured.title} — featured research cover`}
                  ...
                  <div className="p-6 sm:p-8 flex flex-col justify-center">
                    ...
                    <div className="mt-5 flex items-center justify-between">
                      ...
                      <Button asChild size="sm" variant="ghost" className="text-[hsl(var(--brand))] hover:text-[hsl(var(--brand))]">
                        <Link to={`/research/${featured.slug}`} aria-label={`Read editor's pick: ${featured.title}`}>Open report <ArrowRight className="ml-1 h-4 w-4" /></Link>
                      </Button>
                    </div>
                  </div>
                </div>
            ...
            <aside className="lg:col-span-4 flex flex-col gap-6">
              ...
              <div className="surface p-6 flex-1">
                ...
                <Link to="/about" className="mt-3 inline-flex items-center text-sm text-[hsl(var(--brand))] font-medium" aria-label="Read our research methodology">
                  Our methodology <ArrowRight className="ml-1 h-3.5 w-3.5" />
                </Link>
              </div>
            </aside>
          </div>

          {/* Latest research — full width */}
          <section>
            ...
            {loading ? (
              <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
                {[0,1,2,3,4,5].map((i) => <div key={i} className="surface h-64 animate-pulse" />)}
              </div>
            ) : filtered.length === 0 ? (
                  ...
                  >
                    {p.image && (
                      <div className="aspect-[16/9] overflow-hidden">
                        <img src={p.image} alt={`${p.title} — ${p.category} research`}
